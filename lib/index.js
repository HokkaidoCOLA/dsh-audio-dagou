 ...
 */
function realpathDeepest(input) {
    let current = input;
    const rest = [];
    while (true) {
        try {
 ...
 */
export function isPathContained(workspaceRoot, targetPath) {
 ...
 */
export function isWorkspacePath(cwd, targetPath, workspaceRoots) {
    if (cwd !== undefined && isPathContained(cwd, targetPath))
        return true;
    return workspaceRoots.some((root) => isPathContained(root, targetPath));
}
/** 从工具 arguments 里取 `read` 的 file_path；缺失或非字符串时返回 null。 */
function readRequestedFilePath(argumentsValue) {
    if (typeof argumentsValue !== 'object' || argumentsValue === null)
        return null;
    const filePath = argumentsValue.file_path;
 ...
 */
function spawnOnce(sound, maxMs, onDone) {
    const done = (() => {
        let fired = false;
        return () => {
            if (!fired) {
 ...
 */
function playSequence(sounds, gapMs = 150, maxMs = 0) {
